...
def upload_single_batch(client: OpenAI, file_path: Path) -> Optional[str]:
    """
...
def get_jsonl_files(directory: Path, limit: int = 50) -> List[Path]:
    """
    Get a list of `.jsonl` files in the specified directory, up to a specified limit.
    The files are ordered by their names. Skips files that already have a
    corresponding info file.

    Args:
        directory (Path): The directory to search for `.jsonl` files.
        limit (int): The maximum number of files to return.

    Returns:
        List[Path]: A list of paths to `.jsonl` files, ordered by name,
        excluding files with existing info files.
    """
    # Get all `.jsonl` files in the directory
    jsonl_files = list(directory.glob("*.jsonl"))

    # Sort the files by name
    jsonl_files_sorted = sorted(jsonl_files, key=lambda x: x.name)

    # Filter out files that already have a corresponding info file
    filtered_files = []
    for jsonl_file in jsonl_files_sorted:
        info_file_path = get_info_file_path(jsonl_file)
...
def get_info_file_path(jsonl_file: Path) -> Path:
    """
    Generate the corresponding info file path for a given `.jsonl` file.

    Args:
        jsonl_file (Path): The path to the `.jsonl` file.

    Returns:
        Path: The path to the corresponding info file.
    """
    return jsonl_file.with_stem(jsonl_file.stem + "_info").with_suffix(".json")
...
if __name__ == "__main__":
    ...
    # (skipping those with existing info files)
    jsonl_files = get_jsonl_files(batch_files_path, limit=5)

    for job_file_path in jsonl_files:
        # Generate the corresponding info file path
        job_info_file_path = get_info_file_path(job_file_path)

        # Upload the batch file
        file_id = upload_single_batch(client, job_file_path)
        if file_id:
            print(f"Batch file {job_file_path.name} uploaded with ID: {file_id}")

            # Create a batch job for the uploaded file
            batch_job = create_batch_job(
                client=client,
                input_file_id=file_id,
                metadata={"description": job_file_path.name},
            )
            if batch_job:
            ...
            print(f"Failed to upload the batch file {job_file_path.name}.")
